...
Map::Map(RenderSystem* t_rs)
{
	for (int i = 0; i < roomsNum; i++)
	{
		map.push_back(Room(rand() % 7 + 5, rand() % 7 + 5, Vector2((rand() % 38 + 0) * 50, (rand() % 38 + 0) * 50),t_rs)); // Pushes the rooms to a map vector 50 is hardcoded needs to be the tile size
	}
}
...
void Map::CreateMap(RenderSystem* t_rs)
{
	...
	for (int i = 0; i < map.size(); i++)
	{
		...
		{
			map.at(j).checkForOverlap(map.at(i).tileList,t_rs); // the list thats passed is the one that is being changed. and the one calling the function is the one tiles are being deleted from
		}
	}
